...
#define QUICKJS_CHANNEL_SENDNATIVE 2;
    ...
    DLLEXPORT void jsDebuggerWaitConnection(JSContext *ctx, const char *address)
    {
        js_debugger_wait_connection(ctx, address);
    }

    DLLEXPORT void jsDebuggerConnect(JSContext *ctx, const char *address)
    {
        js_debugger_connect(ctx, address);
    }

    DLLEXPORT void jsDebuggerCooperate(JSContext *ctx)
    {
